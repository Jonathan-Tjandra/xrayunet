...
def train(args):
    ...
    train_loader = DataLoader(
        ...
        num_workers=args.num_workers, pin_memory=True, collate_fn=dual_view_collate,
    )
    ...
    criterion  = CombinedLoss(
        bce_weight=args.bce_weight, w_2d=args.w_2d, w_3d=args.w_3d
    ).to(device)
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            for fro_img, lat_img, fro_mask, lat_mask, vol_gts, ct_paths, mask_paths, nums in pbar:
                ...
                for i in range(fro_img.shape[0]):
                    ...
                    vol_gt_i = vol_gts[i].to(device)

                    # DRR geometry — created fresh each sample, destroyed after
                    ...
                    norm = float(args.num_augs * fro_img.shape[0])
                    ...
                    for _ in range(args.num_augs):
                        ...
                        loss, loss_2d, loss_3d = criterion(
                            logits_f, logits_l, aug_fro_m, aug_lat_m,
                            src_F, tgt_F, src_L, tgt_L, drr, vol_gt_i, device,
                        )
                        (loss / norm).backward()
                ...
                del fro_img, lat_img, fro_mask, lat_mask
                torch.cuda.empty_cache()
                if len(losses) % 5 == 0:
                    gc.collect()
                ...
                wandb.log({"batch/loss": total_loss, "batch/grad_norm": grad_norm, "epoch": epoch})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-augs",       type=int,   default=1,
                        help="Kornia augmentation repeats per sample per batch step")
